...
function Nanofetcher () {
  ...
  assert.notEqual(
    ...
    'nanofetcher',
    'nanofetcher: set Component.prototype.constructor = Component'
  )
  assert.ok(
    !this.constructor.prototype.hasOwnProperty('createElement'),
    'nanofetcher: createElement should not be defined'
  )
  assert.ok(
    !this.constructor.prototype.hasOwnProperty('identity'),
    'nanofetcher: identity should be a static method'
  )
...
Nanofetcher.prototype._createElement = function () {
  ...
  assert.equal(
    typeof id,
    'string',
    'nanofetcher.render: Component.identity should return type string'
  )
  ...
  if (this._status == STATUS.FETCHED || this._status == STATUS.FETCHING) {
    assert.ok(
      id == this._dataID || this._dataID == this._createdID,
      'nanofetcher.render: called with different arguments than nanofetch.prefetch'
    )
    if (id != this._dataID) {
      this._data = null
      this._dataID = null
...
Nanofetcher.prototype._handleFetch = function (id, cb) {
  ...
  if (promise && typeof promise.then === 'function') {
    ...
    promise.then(data => _cb.call(this, null, data)).catch(_cb)
  }
...
Nanofetcher.prototype.prefetch = function () {
  ...
  assert.equal(
    typeof id,
    'string',
    'nanofetcher.prefetch: Component.identity should return type string'
  )
  ...
  if (this._status == STATUS.FETCHED || this._status == STATUS.FETCHING) {
    assert.ok(
      id == this._dataID || this._dataID == this._createdID,
      'nanofetcher.prefetch: called twice with different arguments'
    )
    if (id != this._dataID) {
      this._data = null
      this._dataID = null
  ...
  if (this._status == STATUS.FETCHED) {
    if (this._promisify) {
      ...
      assert.equal(
        typeof maybeCb,
        'function',
        'nanofetcher.prefetch: last argument must be callback'
      )
      return maybeCb()
    }
